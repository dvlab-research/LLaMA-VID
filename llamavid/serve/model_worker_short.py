...
class ModelWorker:
    ...
    def load_model_from_cpu(self):
        ...
        self.model.get_vision_tower().cuda()
    ...
    def generate_stream(self, params):
        ...
        if len(images) > 0 and len(videos) == 0:
            yield json.dumps({"text": ori_prompt + "Please switch to \'llama-vid-vicuna-7b-long\' to chat with movie. After switch, you can clear the conversaton then retry.", "error_code": 0}).encode() + b"\0"
            return
        ...
        if len(frame_idx) > 180:
            self.dump_model_to_cpu()
            yield json.dumps({"text": ori_prompt + "We are sorry that, currently the maximum length of customized short videos is limited to 3 minutes.", "error_code": 0}).encode() + b"\0"
            return

        video = vr.get_batch(frame_idx).asnumpy()
        video = image_processor.preprocess(video, return_tensors='pt')['pixel_values']
        video = video.to(device=model.device, dtype=model.dtype)
